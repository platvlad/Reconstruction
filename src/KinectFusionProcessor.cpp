...
DWORD KinectFusionProcessor::Update()
{
	...
	if (!bStopProcessing)
    {
        ...
        else
        {
            if (m_bKinectFusionInitialized && m_bHaveValidCameraParameters)
            {
                ...
                bool processSucceed = ProcessDepth();
				std::cout << "process succeed" << processSucceed << std::endl;
                LeaveCriticalSection(&m_lockVolume);
...
INT64 KinectFusionProcessor::GetElapsedTime()
{
	return elapsedTime;
}
...
HRESULT KinectFusionProcessor::InitializeKinectFusion()
{
    ...
    if (nullptr == m_pDepthVisibilityTestMap)
    {
		std::cout << "Failed to initialize Kinect Fusion depth points visibility test buffer." << std::endl;
        SetStatusMessage(L"Failed to initialize Kinect Fusion depth points visibility test buffer.");
        return E_OUTOFMEMORY;
    }
    ...
    if (nullptr == m_pDepthDistortionMap)
    {
		std::cout << "Failed to initialize Kinect Fusion depth image distortion buffer." << std::endl;
        SetStatusMessage(L"Failed to initialize Kinect Fusion depth image distortion buffer.");
        return E_OUTOFMEMORY;
    }
    ...
    if (nullptr == m_pDepthDistortionLT)
    {
		std::cout << "Failed to initialize Kinect Fusion depth image distortion Lookup Table." << std::endl;
        SetStatusMessage(L"Failed to initialize Kinect Fusion depth image distortion Lookup Table.");
        return E_OUTOFMEMORY;
    }
...
HRESULT KinectFusionProcessor::GetKinectFrames(bool &colorSynchronized)
{
	...
	//middle depth frame
	/*IDepthFrame* midDepthFrame = NULL;
	UINT16* frameData = new UINT16[424 * 512];
	hr = m_pDepthFrameReader->AcquireLatestFrame(&midDepthFrame);
	if (FAILED(hr))
	{
		SafeRelease(midDepthFrame);
        SetStatusMessage(L"Kinect depth stream get frame call failed.");
        return hr;
	}
	midDepthFrame -> CopyFrameDataToArray(424*512, frameData);
	...
	for (int i = 0; i < 4; ++i)
	{
		...
		UINT16* frData = new UINT16[424 * 512];
		pDepthFrame->CopyFrameDataToArray(424 * 512, frData);
		for (int i = 0; i < 424*512; ++i)
		{
			frameData[i] += 0.2 * frData[i];
		}
	}
	...
	//copy and remap depth
        ...
        {
            const UINT id = m_pDepthDistortionLT[i];
            *pDepth = id < bufferLength? frameData[id] : 0;
            *pRawDepth = frameData[i];
        }*/
	...
	std::cout << "process started " << processStarted << std::endl;
	if ((!m_cFirstDepthFrameTimeStamp) && (processStarted))
	{
		m_cFirstDepthFrameTimeStamp = currentDepthFrameTime;
	}

	if (m_cFirstDepthFrameTimeStamp)
		elapsedTime = currentDepthFrameTime - m_cFirstDepthFrameTimeStamp;
    return hr;
}
...
HRESULT KinectFusionProcessor::TrackCameraAlignPointClouds(Matrix4 &calculatedCameraPose, FLOAT &alignmentEnergy)
{
    ...
    if (!FAILED(tracking))
    {
            ...
            m_paramsCurrent.m_fMaxRotationDelta))
        {
            ...
            hr = E_NUI_FUSION_TRACKING_ERROR; //-2097083414
...
bool KinectFusionProcessor::ProcessDepth()
{
	...
	std:: cout << "E_NUI_FUSION_TRACKING_ERROR" << tracking << std::endl;
